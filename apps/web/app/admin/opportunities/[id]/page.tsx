...
import { getAdminOpportunityById } from "@/services/opportunities";

type AdminOpportunityDetailPageProps = {
  params: Promise<{
    id: string;
  }>;
};

export default async function AdminOpportunityDetailPage({ params }: AdminOpportunityDetailPageProps) {
  const { token } = await requireAdminSession();

  const { id } = await params;
  const opportunity = await getAdminOpportunityById(id, token);
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1080px] flex-col gap-8 px-[2vw] pb-16 pt-[14vh]">
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-[680px] text-[18px] leading-[1.45] text-[#4f5762]">
              Trenutno uredjujes: <strong>{opportunity.title}</strong>
            </p>
          </div>
          <NavigationButton
      ...
      <AdminOpportunityEditor initialOpportunity={opportunity} />
    </main>
  );
}
